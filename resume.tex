...
\begin{itemize}
  ...
  % \item Increased the cleanup's team yearly resource budget by \$60,000, by billing the cleanup resource costs with projects using them instead of the cleanup team incurring the costs
  \item Migrated $\sim$30,000 servers from code-as-config alerting configurations to declarative formats, simplifying infrastructure automation and increasing system reliability % deprecated custom_rules.alert files in Pod in favor of gmon. Only 8% of users had customisations, so we simply deleted them for the remaining 92% of users and stopped the generation of customisations for greenfield nodes
